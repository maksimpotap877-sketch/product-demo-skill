...
Переносимый Skill для демонстраций веб-продуктов с русской озвучкой и монтажом Remotion. Рабочая цепочка: **изучение кода → понимание продукта → проверка сценария → запись сайта → озвучка → монтаж → MP4 → QA**.
...
Проверенная среда — Windows. Нужны Node.js 22+, npm, FFmpeg/ffprobe, Chromium либо Microsoft Edge; Python 3.10+ нужен для `edge-neural`. Установка компонентов: [первый запуск](docs/GETTING_STARTED_RU.md).
...
`setup` проверяет среду и при необходимости пытается установить браузер Playwright. FFmpeg, Node и Python устанавливаются отдельно; источники и проверка приведены в инструкции. При доступном Edge загрузка Chromium не обязательна. TTS использует изолированное пользовательское Python-окружение.
